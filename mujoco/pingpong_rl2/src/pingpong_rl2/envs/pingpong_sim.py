...
class PingPongSim:
    ...
    def set_ball_velocity(
        ...
        # free joint의 translational/angular qvel 구간에 직접 속도를 쓰고 forward로 derived state를 갱신한다.
    ...
    def reset(
        ...
        # keyframe 0 home pose로 되돌린 뒤, 공은 지정 위치 또는 racket 위 reset 분포 위치에 다시 둔다.
    ...
    def spawn_ball(
        ...
        # ball free joint의 위치/quaternion/속도를 직접 세팅해 임의 상태에서 episode를 시작한다.
    ...
    def reset_ball_above_racket(
        ...
        # racket center 기준 XY offset과 높이를 더해 공 spawn 위치를 만든다.
    ...
    def _matching_contact(self, contact_geoms: tuple[str, str]) -> dict[str, object] | None:
        # MuJoCo contact list에서 지정 geom pair를 찾고, normal 방향을 racket->ball 기준으로 정규화한다.
    ...
    def ball_robot_body_contact(self) -> str | None:
        # floor/racket 자체를 제외하고 공이 로봇 body와 닿으면 실패 원인에 body 이름을 남긴다.
        for index in range(self.data.ncon):
            contact = self.data.contact[index]
    ...
    def failure_reason(
        ...
        # 물리 발산, 바닥/로봇 접촉, 위치 bounds, 속도 limit을 순서대로 검사한다.
    ...
    def step(
        ...
        # 일반 step은 controller target을 적용한 뒤 control_dt에 맞춘 MuJoCo substep만 진행한다.
    ...
    def step_with_contact_trace(
        ...
        for substep_index in range(1, step_count + 1):
            # 첫 contact가 관측된 substep에서 pre/contact 상태를 고정해 이후 post-contact와 비교한다.
            mujoco.mj_step(self.model, self.data)
            ball_position = self.ball_position
            ...
            if contact_trace["contact_observed"]:
                # contact 이후 최대 5 substep의 공 속도를 샘플링해 반발이 안정됐는지 확인할 수 있게 한다.
