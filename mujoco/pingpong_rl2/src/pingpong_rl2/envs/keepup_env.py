...
class PingPongKeepUpEnv:
    ...
    def __init__(
        ...
        # 성공 판정과 follow-up 보상은 다음 descending intercept가 다시 칠 수 있는지에 집중한다.
        ...
        # contact-frame mode는 접촉 좌표계 target, desired outgoing velocity, residual action limit을 함께 쓴다.
        ...
        # 여기부터는 CLI/preset 조합이 env 계약을 벗어나지 않았는지 한 번에 검증한다.
    ...
    def reset(
        ...
        # reset info는 vector env reset_infos와 분석 로그에서 시작 조건을 재현하는 데 사용된다.
    ...
    def step(self, action: Sequence[float]) -> tuple[np.ndarray, float, bool, bool, dict[str, object]]:
        ...
        # 위치 target은 일반 position, strike contract, contact-frame mode별로 서로 다른 helper가 만든다.
        ...
        # contact event와 failure/success reason을 먼저 확정해야 reward와 termination을 일관되게 계산할 수 있다.
        ...
        next_intercept_metrics = self._next_intercept_metrics()
        # stable cycle은 useful contact 이후 다음 공이 다시 쉬운 intercept로 돌아오는지를 별도 누적한다.
        ...
        # 다음 step에서 contact edge와 action delta를 계산할 수 있도록 episode-local memory를 갱신한다.
