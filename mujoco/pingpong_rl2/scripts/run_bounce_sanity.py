...
from pingpong_rl2.defaults import DEFAULT_BALL_HEIGHT, DEFAULT_BOUNCE_EPISODES, DEFAULT_BOUNCE_MAX_STEPS
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--episodes", type=int, default=DEFAULT_BOUNCE_EPISODES)
    parser.add_argument("--max-steps", type=int, default=DEFAULT_BOUNCE_MAX_STEPS)
    parser.add_argument("--ball-height", type=float, default=DEFAULT_BALL_HEIGHT)
    parser.add_argument("--xy-offset", type=float, nargs=2, metavar=("DX", "DY"), default=(0.0, 0.0))
    parser.add_argument(
        "--ball-velocity",
        type=float,
        nargs=3,
...
def main() -> None:
    ...
    # LINK: mujoco/pingpong_rl2/src/pingpong_rl2/envs/pingpong_sim.py:13
    args = parse_args()
    sim = PingPongSim()
    racket_first_count = 0
    for episode in range(1, args.episodes + 1):
        sim.reset()
        sim.reset_ball_above_racket(height=args.ball_height, xy_offset=args.xy_offset, velocity=args.ball_velocity)
